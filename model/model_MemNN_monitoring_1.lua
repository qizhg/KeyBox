...
function g_build_model()
    ...
    local mem_out_monitoring  = build_model_memnn_monitoring(dummy_context, mem_state_monitoring)
    ...
    comm_decoder:add(nn.Linear(g_opts.hidsz, 2))
    comm_decoder:add(nn.LogSoftMax())
    local matching_logp = comm_decoder(comm_in)
    
    --local matching_logp = nn.Identity()(comm_in)

    local model = nn.gModule({mem_state_monitoring, dummy_context, comm_in, Gumbel_noise}, 
                             {Gumbel_SoftMax, matching_logp})
