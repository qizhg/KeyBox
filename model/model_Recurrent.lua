...
function g_build_model()
    ...
    local prev_mem_out = nn.Identity()() --(#batch, ask_hidsz)
    ...
    local hid, cell = build_lstm(prev_mem_out, prev_hid, prev_cell, g_opts.hidsz, g_opts.hidsz)
    ...
    local model = nn.gModule({mem_state, prev_mem_out, prev_hid, prev_cell}, 
                            {action_prob, baseline, mem_out, hid, cell})
