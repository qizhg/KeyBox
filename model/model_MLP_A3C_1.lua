...
function g_build_model()
	...
	local hid_acting = nn.CAddTable()({comm_in2hid, input2hid_acting})
	local action = nn.Linear(g_opts.hidsz, g_opts.nactions)(hid_acting)
    local baseline = nn.Linear(g_opts.hidsz, 1)(hid_acting)
    local action_prob = nn.LogSoftMax()(action)

    local model = nn.gModule({input_monitoring, input_acting, comm_in}, {out_monitoring, action_prob, baseline})
    return model
end
