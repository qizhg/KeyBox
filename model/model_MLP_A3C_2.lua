...
local function mlp_acting( input )
    ...
    if g_opts.nlayers > 1 then
        ...
        a:add(nonlin())
        a:add(nn.BatchNormalization(g_opts.hidsz))
        hidstate = a(input)
    else
        error('wrong nlayers')
    end
...
function g_build_model()
    ...
    local hid_final_acting = nn.JoinTable(2)({input2hid_acting, input2hid_monitoring})
    ...
    local model = nn.gModule({input_monitoring, input_acting}, 
