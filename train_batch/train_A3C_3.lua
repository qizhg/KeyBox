...
function train_batch()
    ...
    local prev_hid_final_monitoring = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0)--4
    local prev_cell_final_monitoring = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --5
    local prev_hid_final_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --8
    local prev_cell_final_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0)--9
    ...
    for t = 1, g_opts.max_steps do
        ...
        input[t][3] = mem_state_monitoring:clone()
        input[t][4] = prev_hid_final_monitoring:clone()
        input[t][5] = prev_cell_final_monitoring:clone()
        input[t][6] = dummy:clone()
        input[t][7] = mem_state_acting:clone()
        input[t][8] = prev_hid_final_acting:clone()
        input[t][9] = prev_cell_final_acting:clone()

        local out = g_model:forward(input[t])

        action[t] = sample_multinomial(torch.exp(out[2])) 
        prev_hid_final_monitoring = out[4]:clone()--4
        prev_cell_final_monitoring = out[5]:clone()--5
        prev_hid_final_acting = out[6]:clone()--6
        prev_cell_final_acting = out[7]:clone()--7
    ...
    local grad_hid_final_monitoring = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0)--4
    local grad_cell_final_monitoring = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --5
    local grad_hid_final_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --6
    local grad_cell_final_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0)--7
    ...
    for t = g_opts.max_steps, 1, -1 do
        if active[t] ~= nil and active[t]:sum() > 0 then
            ...
            grad_table[4] = grad_hid_final_monitoring:clone()
            grad_table[5] = grad_cell_final_monitoring:clone()
            grad_table[6] = grad_hid_final_acting:clone()
            grad_table[7] = grad_cell_final_acting:clone()
