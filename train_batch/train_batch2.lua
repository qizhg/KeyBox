...
function train_batch()
    ...
    local prev_mem_out_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --9
    local prev_hid_context_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --10
    local prev_cell_context_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --11
    local prev_hid_comm_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --12
    local prev_cell_comm_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --13
    ...
    for t = 1, g_opts.max_steps do
        ...
        input[t][8] = mem_state_acting:clone()
        input[t][9] = prev_mem_out_acting:clone()
        input[t][10] = prev_hid_context_acting:clone()
        input[t][11] = prev_cell_context_acting:clone()
        input[t][12] = prev_hid_comm_acting:clone()
        input[t][13] = prev_cell_comm_acting:clone()
        ...
        prev_mem_out_acting = out[9]:clone()--9
        prev_hid_context_acting = out[10]:clone()--10
        prev_cell_context_acting = out[11]:clone()--11
        prev_hid_comm_acting = out[12]:clone()--12
        prev_cell_comm_acting = out[13]:clone()--13
    ...
    local grad_mem_out_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --9
    local grad_hid_context_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --10
    local grad_cell_context_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --11
    local grad_hid_comm_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --12
    local grad_cell_comm_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.hidsz):fill(0) --13
    ...
    for t = g_opts.max_steps, 1, -1 do
        if active[t] ~= nil and active[t]:sum() > 0 then
            ...
            grad_table[9] = grad_mem_out_acting:clone()
            grad_table[10] = grad_hid_context_acting:clone()
            grad_table[11] = grad_cell_context_acting:clone()
            grad_table[12] = grad_hid_comm_acting:clone()
            grad_table[13] = grad_cell_comm_acting:clone()
