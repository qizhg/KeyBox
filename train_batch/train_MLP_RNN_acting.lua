...
function train_batch(num_batch)
    ...
    for t = 1, g_opts.max_steps do
        ...
        input[t] = {}
        input[t][1] = batch_input_mlp(batch, active[t], t)
        input[t][2] = prev_hid_acting:clone()
        input[t][3] = prev_cell_acting:clone()

        local out = g_model:forward(input[t])
        action[t] = sample_multinomial(torch.exp(out[1]))

        prev_hid_acting = out[3]:clone()
        prev_cell_acting = out[4]:clone()
    ...
    for t = g_opts.max_steps, 1, -1 do
        if active[t] ~= nil and active[t]:sum() > 0 then
            ...
            grad_table[3] = grad_hid_acting:clone()
            grad_table[4] = grad_cell_acting:clone()
