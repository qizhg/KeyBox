...
function train_batch(num_batch)
    ...
    local action = {}
    local q_value = {}
    local q_value_target = {}

    local eps = g_opts.eps_start - num_batch*g_opts.eps_start/g_opts.eps_endbatch
    eps = math.max(g_opts.eps_end, eps)
    ...
    for t = 1, g_opts.max_steps do
        ...
        local out_target = g_model_target:forward(input[t])
        q_value[t] = out:clone()
        q_value_target[t] = out_target:clone()
        action[t] = batch_eGreedy(q_value[t], eps)
    ...
    for t = g_opts.max_steps, 1, -1 do
        if active[t] ~= nil and active[t]:sum() > 0 then
            ...
            --grad_q_acting: r + max_a Q(s',a) - Q(s,a)
            local r = reward[t]:clone():cmul(active[t]):view(-1,1)
            local q_target = torch.Tensor(g_opts.batch_size,1):zero()
            if active[t+1] ~= nil and active[t+1]:sum() > 0 then
                local max, indices =  torch.max(q_value_target[t+1], 2)
                q_target = max:clone():cmul(active[t+1])
            end
            local q = q_value[t]:gather(2, action[t]):clone():cmul(active[t])
            local td_error = r + q_target:mul(g_opts.gamma) - q
            local grad_q_acting = torch.Tensor(#batch * g_opts.nagents, g_opts.nactions):fill(0)
            grad_q_acting:scatter(2, action[t], td_error:neg())
            
            --normalize with div(#batch_size)
            grad_q_acting:div(g_opts.batch_size)
            
            g_model:backward(input[t], grad_q_acting)  
